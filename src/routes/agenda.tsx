...
function ProtectedAgendaPage() {
  const { currentProfile, isLoading, isAuthenticated, subscriptionSummary, setCurrentProfile } =
    useAuth();
  ...
  useEffect(() => {
    if (!isLoading && isAuthenticated && !currentProfile) {
      const isMultiPerfil =
        subscriptionSummary?.plan?.permiteMultiplosPerfis === true &&
        (subscriptionSummary?.subscription?.totalPerfisPermitidos ?? 1) > 1;

      // O seletor estilo Netflix só deve abrir se for plano multi-perfil contratado
      if (isMultiPerfil) {
        navigate({ to: "/perfis" });
      } else {
        // No Plano Essencial, Plano Avançado ou Trial, auto-seleciona o perfil primário sem desvio
        apiClient
          .getProfiles()
          .then((profiles) => {
